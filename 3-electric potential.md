...
Consider a point charge q. To find the electric potential at a point in space, at position $\vec{r}$ in space with respect to q, imagine bringing a unit positive charge from infinity to this point. Let the electric potential at this point be $V_r$. In case of a unit positive charge, the force vector $\vec{F}$ is equal to the electric field $\vec{E}$ . Since work is defined as the dot product of force and displacement, and the distance between the source charge and the test charge keeps changing as the charge moves inward, this work has to be calculated using an integral. 
